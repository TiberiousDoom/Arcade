...
- **[choke-point.html](games/choke-point/choke-point.html)** is the fullest shell: the standard layout/rotation/pause + help + audio + scores wiring from Hull Breach, plus a **`#controls` strip** (tower palette, a four-button wave row, and a full-width armoury button), a tap-a-tower targeting/sell popup, and the armoury modal. It is also the only game using `makeFit`'s **`fillWidth`** — paired with `body.scrolls` from the theme, because that mode can push furniture below the fold and the default `overflow:hidden` would make it unreachable. Towers draw as bare rings until `towerReady` says something is near, then grow a hub and slide a barrel out.
- **[engine.test.js](games/choke-point/engine.test.js)** — board/transpose invariant → path geometry → building → levelling → armoury → difficulty → tower firing (range/cooldown/splash/slow) → targeting priority → enemies (movement/kill/leak) → waves/rush → rotation (lossless round-trip) → full-run sanity.
...
- **Render test but no standalone build** — [render-test.mjs](games/choke-point/render-test.mjs) inlines the shell in memory, same as Hull Breach and Feedline. It covers the full enemy roster with every trait cue, all three tower types across the level range, both idle and deployed tower art, the armoury driven through real DOM clicks, the palette staying put when you go broke, and a live run deep enough to include Shell, Phase and Patch.
