...
- **Convergence engages by range, and its tiers buy reach rather than degree.** Inside the engagement distance the mounts focus on the target (which is what makes a close command ship killable — 9.9s to 3.5s measured); outside it the flat `FOCUS_RANGE` fan is left alone, because five mounts on one distant point sweep a dense column. Interpolating the *degree* of convergence instead made the middle tiers worse than the low ones. Don't "fix" either property without re-reading DECISIONS 2026-08-08.
...
- **[engine.js](games/choke-point/engine.js)** — pure logic, seeded LCG only.
  ...
  - **Progression is a pure function over a plain object**: `newProgress`/`sanitizeProgress`/`routeUnlocked`/`unlockedRoutes`/`difficultyUnlocked`/`recordWin`. Circuits unlock one at a time per difficulty; Easy is the only difficulty open until a win, then Medium, then Hard. The *shell* stores it (`PROGRESS_KEY`), same split as the armory, and `readDifficulty` re-checks the gate so a stored value can't smuggle a locked difficulty back in. `DEFAULT_DIFFICULTY` is `easy` and `START_COMPONENTS`/`START_INTEGRITY` derive from it — don't hardcode a difficulty in either.
