...
- Core simulation model, useful when tracing a bug:
  ...
  - `setGunType(w, mount, type)` charges a **retrofit** (`retrofitCost`) on top of the one-time `unlock` research; pass `free: true` only when building a mount rather than converting one.
...
- **[engine.js](games/choke-point/engine.js)** — pure logic, seeded LCG only.
  ...
  - Public API mirrors the others plus TD verbs: `canBuild`/`buildTower`/`sellTower`/`setPriority`, `buyClassUpgrade`/`classCost`, `startWave`/`rushWave`, `cellAt`/`towerAt`, `towerReady`, `relayout`.
