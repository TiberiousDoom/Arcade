...
- **[build.mjs](games/serpent-battery/build.mjs)** is that generator. It inlines each ES module as an IIFE returning *all* its top-level names (the shell reaches for a few that aren't formally exported), and throws if any JS `import` survives. Note its stray-import guard matches `^\s*import\s` specifically — a plain substring check would trip over CSS's legitimate `@import url(...)`.
...
- **`shared/theme.css`** — the cabinet look. Shells link it and override only the custom properties (`--accent`, `--accent-hot`, `--accent-ink`, `--board-max`). **Don't redeclare shared rules in a game's local `<style>`** — add a property hook to the theme instead.
...
- **`shared/fx.js`** — `makeFx(...)` for particles and the screen-flash value. Breakout and Snake use it. **Serpent Battery deliberately does not** — its bits/floaters live on the world and are stepped inside its engine, and rewiring that was judged not worth the churn.
