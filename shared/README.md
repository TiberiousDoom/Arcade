...
- **[audio.js](audio.js)** — `makeAudio()` synthesizes all sound effects with
  WebAudio (no files). Guarded against no-AudioContext environments and
  remembers mute in localStorage; the mute *button* now lives in `menu.js`.
- **[scores.js](scores.js)** — `best()` / `submit()` for local personal bests.
- **[menu.js](menu.js)** — `makeMenu({ stage, audio, title, rows, notes, lore, onLevels })`,
  the single corner button + panel every game uses for pause, mute, and
  instructions. Replaced three separate always-visible buttons (`pause.js`,
  `help.js`, and `audio.js`'s old `mountAudioToggle`) that overlapped tappable
  board area. Opening the panel pauses the run — shells still gate their frame
  loop on the returned `{ paused, pause(), resume(), toggle() }`, unchanged
  from the old `pause.js` contract. `onLevels` is optional; only Hull Breach
  passes it, to add a "Levels" entry.
- **[fx.js](fx.js)** — `makeFx({ reduce, gravity })` gives particles and a
  screen-flash value. Used by Hull Breach and Feedline. **Flak Battery does not
...
- **[resume.js](resume.js)** — storage for mid-run saves. The *engines* own what
  ...
  Battery and Hull Breach; **not** Feedline, which is one-life score-attack.
- **[glow.js](glow.js)** — the vector/CRT art primitives: `glowStroke` (the
  ...
  Feedline and Choke Point. Five things to know before using it:
  ...
  - **Extrusion suits discrete objects on a dark board**, not the tip of an
    ...
    Feedline's head when it was still a disc.
  ...
  - **Additive compositing cannot darken.** `glowStroke` and `glowDot` are
    ...
    this was understood, and Feedline's pupils are `inkDot` for the same reason.
