/* Generates flak-battery-standalone.html — a single portable file with the
   ...
   Flak Battery is the only game with one. The other three get render tests
   by inlining in memory instead, which is the same safety net without a
   checked-in file to keep in sync.

   Run:  node games/flak-battery/build.mjs
*/
...
const html = inlineGame(join(here, 'flak-battery.html'));
writeFileSync(join(here, 'flak-battery-standalone.html'), html);
console.log('wrote flak-battery-standalone.html');
