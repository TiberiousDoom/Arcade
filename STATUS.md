...
### The art round — ported in v39

**All five portraits are redesigned and in the shell.** The bench is no longer
ahead of the game; it is generated from it (`node tools/gun-bench.mjs`).

- **Mounts are per gun type** (`towed` / `deck` / `bunker` / `mound`), and so is
  the trunnion height. What a gun stands on is read before its barrel, so
  outriggers, a bolted deck plate, an armoured casemate and a low earthwork are
  four different weapons at a glance.
- **Cannon** — cruciform outriggers on jack pads, breech behind the trunnion,
  recoil cradle, equilibrator, tapered tube. Shipped in v38.
- **Railgun** — deck plate, turret drum, mount body, an *elevating* housing
  between barrel and counterweight, and an open rail truss in three-quarter
  view: coils along the top channel, near flank closed.
- **Autocannon** — bunker casemate with bolted plates and a ground skirt, a
  squat wide turret with ammunition boxes, and a four-barrel rotary cluster.
- **Ion cannon** — a thin rod inside three arms bent around it, on a low
  earthwork drawn in two halves: the near parapet paints *after* the gun, so it
  stands in the emplacement rather than on it. That is what the `foreground`
  hook in `drawCannonPortrait` exists for.
- **Mortar** — deliberately untouched. It kept the towed mount and its own
  steeper elevation, and it still reads as itself beside the other four.
